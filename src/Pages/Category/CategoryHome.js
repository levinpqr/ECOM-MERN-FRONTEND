...
const CategoryHome = (props) => {
    ...
    const initialize = React.useCallback(async () => {
        ...
        if (!slug) return;
        setLoading(true);
        const categFilter = { limit: 1, where: { slug } };
        const res = await getCategories(categFilter).catch(toastError);
        if (res && isNonEmptyArray(res.data)) setCategory({ ...res.data[0] });
        else return setLoading(false);
        const prodFilter = {
            where: { $and: [{ category_str: res.data[0]._id }] },
        };
            ...
            <div className="row">
                <div className="col">
                    <h4 className="text-center p-3 mt-5 mb-5 display-4 jumbotron">
                        {prodCount} Product/s in "{category.name || ""}"
                        category
                    </h4>
                </div>
            </div>
